...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = self.head_proj(query).view(N, S, H, D//H).transpose(1, 2)   # (N, H, S, D/H)
        key = self.head_proj(key).view(N, T, H, D//H).transpose(1, 2)   # (N, H, T, D/H)
        value = self.head_proj(value).view(N, T, H, D//H).transpose(1, 2)   # (N, H, T, D/H)
